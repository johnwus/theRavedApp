...
const BookmarkButton: React.FC<BookmarkButtonProps> = ({
  ...
  const color = useMemo(() => (optimisticBookmarked ? '#6C5CE7' : '#6b7280'), [optimisticBookmarked]);
  ...
  return (
    <TouchableOpacity onPress={handlePress} activeOpacity={0.7} disabled={disabled || loading}>
      {loading ? (
        ...
        <Ionicons name={optimisticBookmarked ? 'bookmark' : 'bookmark-outline'} size={size} color={color} />
      )}
    </TouchableOpacity>
  );
};
