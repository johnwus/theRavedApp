  ...
  reducers: {
    ...
    addConnection: (state, action: PayloadAction<Connection>) => {
      if (action.payload.status === 'accepted') {
        ...
        state.connectionRequests = state.connectionRequests.filter(c => c.id !== action.payload.id);
    ...
    // Search
    setSearchResults: (state, action: PayloadAction<{ users: User[]; posts: any[]; tags: string[] }>) => {
    ...
    // Interaction states
    setFollowingInProgress: (state, action: PayloadAction<{ userId: string; inProgress: boolean }>) => {
