...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    // Messages
    setMessages: (state, action: PayloadAction<{ chatId: string; messages: Message[]; hasMore: boolean }>) => {
    ...
    prependMessages: (state, action: PayloadAction<{ chatId: string; messages: Message[]; hasMore: boolean }>) => {
      const { chatId, messages, hasMore } = action.payload;
    ...
    updateMessage: (state, action: PayloadAction<Message>) => {
      ...
      if (!list) return;
      state.messages[message.chatId] = list.map((m) => (m.id === message.id ? { ...m, ...message, isEdited: true } : m));
    ...
    setMessageStatus: (state, action: PayloadAction<{ chatId: string; messageId: string; status: Message['status'] }>) => {
    ...
    clearTypingIndicators: (state, action: PayloadAction<{ chatId: string }>) => {
      state.typingIndicators = state.typingIndicators.filter((t) => t.chatId !== action.payload.chatId);
    },
    ...
    startUploadingMedia: (state, action: PayloadAction<{ chatId: string; messageId: string }>) => {
      state.uploadingMedia[action.payload.chatId] = { progress: 0, messageId: action.payload.messageId };
    },
    ...
    saveAudioRecording: (state, action: PayloadAction<{ chatId: string; duration: number; uri: string }>) => {
    ...
    setLoadingMoreMessages: (state, action: PayloadAction<{ chatId: string; loading: boolean }>) => {
    ...
    updateChatSettings: (
      state,
      action: PayloadAction<Partial<ChatState['chatSettings']>>
    ) => {
