...
import { registerForPushNotificationsAsync, addNotificationListeners, isNotificationsAvailable, showLocalNotification } from './pushNotifications';
...
export async function initNotifications(options?: InitOptions) {
  ...
  listeners = addNotificationListeners(
    (notification) => {
      ...
      dispatch(setLastNotification({ title: title ?? 'Notification', body: body ?? undefined, data }));
      ...
      if (AppState.currentState === 'active') {
        dispatch(showInAppNotification({ title: title ?? 'Notification', body: body ?? undefined, data }));
      }
    },
    (response) => {
